...
document.addEventListener('DOMContentLoaded', // When the DOM content is fully loaded
    (function () {
...
function formatDate() { // A function to format the date to a usable format for the API request
...
function ShowSearchAlert() { // A function to show the alert box
...
function CloseSearchAlert() { // A function to close the alert box
...
function ColourStyle() { // A function to change the style sheet based off of the local storage variable
...
var Initmodal = document.getElementById("InitCountrySelectModal"); // Get the Initial modal element

function OpenInitCountryModal() { // A function to open the initial modal
...
function InitSelectLang() { // A function to store the elements from the initial modal
...
function HideCards() { // A function to hide all of the article cards
...
function ShowCards() { // A function to show all of the article cards
...
function OpenSettingsModal() { // A function to open the settings modal
...
function CloseSettingsModal() { // A function to close the settings modal
    var Settings = document.getElementById("SettingsModal"); // Store the element for the settings modal
...
function LoadStoredSettings() { // A function to load all of the stored settings and search terms
...
function FilterSubmit() { // A function to submit and save the search options
    ...
    if (FromDate > formatDate()) { // If the From date is after today
...
function SettingsSubmit() { // A function to submit and save the settings
...
function ArticleModalOpen(i) { // A function to show the Article modal
...
function ArticleModalClose() { // A function to close the Article modal
...
function OutputResults(Data) {
    ...
    for (i = 0; i < PageLimit; i++) { // For each number up to the Page Limit
        ...
        var Date = Data.articles[i].publishedAt == null ? 'Unknown' : FormatDate(Data.articles[i].publishedAt);
        var ImageURL = Data.articles[i].urlToImage == null ? 'images/download.png' : CheckImage(Data.articles[i].urlToImage);
...
var ReturnedData; // Declare these variables globally
...
function RetrieveNews(Page, callback) {
    ...
    SelectedCountry = localStorage.getItem("Country"); // Retrieve the Country from the local storage
...
function PageNum(PageNum) {
    return "<li class='page-item'><a id='PageNum" + PageNum + "' class='page-link' onclick='RefreshArticles(" + PageNum + ")'>" + PageNum + "</a></li>";
}
...
function GetCategory(i) { // Gets the category from the array based off of the integer given, ready for use in the XMLHttpRequest
...
function InitialiseCatSelection() { // A function to initialise the flickity carousel
    $('#NavCategoryScroll').flickity({ // Initialise the flickity carousel
        ...
        pageDots: false, // Remove the controls
    ...
    var NavBarCarousel = $carousel.data('flickity');

    NavBarCarousel.on('change', function (index) { // On the carousel change 
...
function CreateSearchCell() { // A function to add the Search cell from the navigation bar
    var $carousel = $('.carousel').flickity(); // Select the carousel
    var NavBarCarousel = $carousel.data('flickity');
...
function RemoveSearchCell() { // A function to remove the Search cell from the navigation bar
    var $carousel = $('.carousel').flickity(); // Select the carousel
    var NavBarCarousel = $carousel.data('flickity');
